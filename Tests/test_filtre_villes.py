...
def test_filtre_par_villes():
    ...
    assert df[df.VILLE=="ALISE-SAINTE-REINE"].applymap(lambda x: {} if isnull(x) else x).eq(filtre_par_villes(df,"ALISE-SAINTE-REINE").applymap(lambda x: {} if isnull(x) else x)).all().all()
  


test_filtre_par_villes()